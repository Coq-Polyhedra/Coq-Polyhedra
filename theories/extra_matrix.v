...
Lemma trmxN (R : zmodType) (m n : nat) (M : 'M[R]_(m,n)): (- M)^T = - M^T.
...
(* QC : there is probably a better name *)
Lemma castmx_mulC (R : realFieldType) (p q p' q' : nat) (e : p' = q) (M : 'M[R]_(p,q)) (N : 'M[R]_(p',q')) :
  (castmx (erefl _, esym e) M) *m N = M *m (castmx (e, erefl _) N).
Proof.
rewrite mulmx_cast.
by rewrite -[N in LHS](castmx_id (erefl _ , erefl _)).
Qed.
...
Context {R : ringType} (m n : nat) (i : 'I_n) (A : 'M[R]_(m,n)).

Lemma colE : col i A = A *m delta_mx i 0.
Proof.
apply/colP=> j; rewrite !mxE (bigD1_ord i) //= mxE !eqxx mulr1n.
by rewrite big1 ?addr0 ?mulr1 // => j' _; rewrite mxE lift_eqF /= mulr0.
Qed.
